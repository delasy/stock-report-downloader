...
async function downloadReportsInfo (): Promise<DownloadReportsInfo[]> {
  const res = await axios.get('https://investor.hp.com/feed/FinancialReport.svc/GetFinancialReportList', {
    headers: {
  ...
  if (reportsData === null) {
    throw new Error('No data returned by API endpoint in HPQ reports handler')
  }
  ...
  for (const reportData of reportsData) {
    ...
    if (reportYear === null) {
      throw new Error('Incomplete HPQ report')
    }

    const reportQuarter: string | null = reportData?.ReportSubType ?? null

    if (reportQuarter === null || !isLongQuarter(reportQuarter)) {
      throw new Error(`Incomplete HPQ ${reportYear.toString()} report`)
    }

    const quarter = toNumQuarter(reportQuarter)
    const reportDocuments: any[] | null = reportData?.Documents ?? null
    const reportUrl = reportDocuments?.find(it => it.DocumentCategory === 'quarterly')?.DocumentPath ?? null

    if (reportUrl === null) {
      throw new Error(`Incomplete HPQ ${yearQuarter(reportYear, quarter)} report`)
    }
...
export default downloadReportsMiddleware('HPQ', downloadReportsInfo, getReportModifiedOn, downloadReportsData)
